...
export class UsersService {
    ...
    async createUser(userBody: UserInputDto): Promise<UserViewDto> {
        ...
        const passHash = await this.cryptoService.createPasswordHash(userBody.password)
        const confirmationCode = uuid()
        const createdUser = await this.usersRepository.createUser(userBody, passHash, confirmationCode)
        return {
